...
public class Triangle extends Shape {
	...
	public void draw(Window window) {
		...
		if (rowIncrement == 0 && colIncrement == 1) {
			for (int i = 0; i <= height; i++) {
				...
				// newRow = rowBase + rowIncrement +
				// i*(int)Math.round(1.0/Math.cos(Math.toRadians(45)));
				// newCol = colBase + colIncrement +
				// i*(int)Math.round(1.0/Math.sin(Math.toRadians(45)));
	...
	public void writeToFile(String fileName) {
		...
		try {
			...
			out.println(rowBase + " " + colBase + " " + height + " " + rowIncrement + " " + colIncrement);
